class SettlementRulesController < ApplicationController
  ...
  def download
    respond_to do |format|
      format.xlsx { response.headers['Content-Disposition'] = "attachment; filename='#{@settlement_rule.name}-加工商结算价目表.xlsx'"}
    end
  end
  ...
  def upload
    if request.post?
      ...
      respond_to do |format|
        format.html { redirect_to @settlement_rule, notice: '结算价格更新完毕.' }
      end
    end
  end
